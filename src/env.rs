...
fn build_grpc2kafka() -> anyhow::Result<ConfigGrpc2Kafka> {
    Ok(ConfigGrpc2Kafka {
        ...
        x_token: optional_env_value("REQUEST_X_TOKEN"),
...
{
    json5::from_str(&json_string).context("failed to parse request {json_string:?}")
}

fn required_env_value(name: &str) -> anyhow::Result<String> {
    env::var(name).context("failed to load env var {name:?}")
}
